...
3. `index.html` = administración (edita Álvaro/Berna). `asesores.html` = solo consulta; es la que se enlaza desde TRUST Asesores.
4. Abrir la URL de Vercel. El pie de página debe decir `Supabase · hh:mm`. Si dice "Modo local", revisar `SUPABASE_URL` / `SUPABASE_KEY` en `index.html`.
